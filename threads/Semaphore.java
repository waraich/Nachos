 ...
 * <li><tt>V()</tt>: increments the semaphore's value, and wakes up one thread waiting in <tt>P()</tt> if possible.
 * </ul>
 * 
 * <p>
 * Note that this API does not allow a thread to read the value of the semaphore directly. Even if you did read the
 * value, the only thing you would know is what the value used to be. You don't know what the value is now, because by
 * the time you get the value, a context switch might have occurred, and some other thread might have called
 * <tt>P()</tt> or <tt>V()</tt>, so the true value might now be different.
 */
