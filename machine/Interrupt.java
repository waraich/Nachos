 ...
 * 
 * <p>
 * In order to emulate the hardware, we need to keep track of all pending interrupts the hardware devices would cause,
 ...
 * 
 * <p>
 * As a result, unlike real hardware, interrupts (including time-slice context switches) cannot occur just anywhere in
 * the code where interrupts are enabled, but rather only at those places in the code where simulated time advances (so
 * that it becomes time for the hardware simulation to invoke an interrupt handler).
 * 
 * <p>
 * This means that incorrectly synchronized code may work fine on this hardware simulation (even with randomized time
 * slices), but it wouldn't work on real hardware. But even though Nachos can't always detect when your program would
...
public final class Interrupt {
    ...
    /**
     * Enable interrupts. This method has the same effect as <tt>setStatus(true)</tt>.
     */
    public void enable() {
        setStatus(true);
    }
