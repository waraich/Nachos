 ...
 * 
 * <p>
 * Do not use any methods in <tt>java.lang.Thread</tt>, as they are not compatible with the TCB API. Most
 * <tt>Thread</tt> methods will either crash Nachos or have no useful effect.
 * 
 * <p>
 * Do not use the <i>synchronized</i> keyword <b>anywhere</b> in your code. It's against the rules, <i>and</i> it can
 * easily deadlock nachos.
 */
public final class TCB {
    ...
    /**
     * Wake up this TCB by setting its <tt>running</tt> flag to <tt>true</tt> and signalling the monitor bound to it.
     * Used in the ping-pong process of
    ...
    /**
     * A reference to the currently running TCB. It is initialized to <tt>null</tt> when the <tt>TCB</tt> class is
     * loaded, and then the first
     * invocation of <tt>start(Runnable)</tt> assigns <tt>currentTCB</tt> a
     * reference to the first TCB. After that, only <tt>yield()</tt> can
     * change <tt>currentTCB</tt> to the current TCB, and only after <tt>waitForInterrupt()</tt> returns.
     * 
     * <p>
     * Note that <tt>currentTCB.javaThread</tt> will not be the current thread if the current thread is not bound to a
     * TCB (this includes the threads created for the hardware simulation).
     */
     ...
     * on each TCB object. TCB objects are removed only in each of the <tt>catch</tt> clauses of <tt>threadroot()</tt>,
     * one of which is always
     * invoked on thread termination. The maximum number of threads in <tt>runningThreads</tt> is limited to
     * <tt>maxThreads</tt> by <tt>start(Runnable)</tt>. If <tt>threadroot()</tt> drops the number of
     ...
     * started in <tt>Machine.main(String[])</tt>. Initialized by <tt>start(Runnable)</tt>, on the basis of whether
     * <tt>currentTCB</tt> has been initialized.
     */
    private boolean isFirstTCB;

    /**
     * A reference to the Java thread bound to this TCB. It is initially <tt>null</tt>, assigned to a Java thread in
     * <tt>start(Runnable)</tt>,
     ...
     * be running. This is an entirely different condition from membership in <tt>runningThreads</tt>, which contains
     * all TCB objects that have
     * started and have not terminated. <tt>running</tt> is only <tt>true</tt> when the associated Java thread ought to
     * run ASAP. When starting or
    ...
    /**
     * Set to <tt>true</tt> by <tt>destroy()</tt>, so that when <tt>waitForInterrupt()</tt> returns in the doomed TCB,
     * <tt>yield()</tt> will know that the current TCB is doomed.
     */
