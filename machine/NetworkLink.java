 ...
 * 
 * <p>
 * The link layer uses the physical layer to provide a packet interface to the network layer. The link layer generally
 * provides unreliable delivery of limited-size packets, but guarantees that packets will not arrive out of order. Some
 * links protect against packet corruption as well. The ethernet protocol is an example of a link layer.
 * 
 * <p>
 * The network layer exists to connect multiple networks together into an internet. The network layer provides globally
 * unique addresses. Routers (a.k.a. gateways) move packets across networks at this layer. The network layer provides
 * unordered, unreliable delivery of limited-size uncorrupted packets to any machine on the same internet. The most
 * commonly used network layer protocol is IP (Internet Protocol), which is used to connect the Internet.
 * 
 * <p>
 * The session/transport layer provides a byte-stream interface to the application. This means that the transport layer
 * must deliver uncorrupted bytes to the application, in the same order they were sent. Byte-streams must be connected
 * and disconnected, and exist between ports, not machines.
 * 
 * <p>
 * This class provides a link layer abstraction. Since we do not allow different Nachos networks to communicate with one
 * another, there is no need for a network layer in Nachos. This should simplify your design for the session/transport
 * layer, since you can assume packets never arrive out of order.
 */
public class NetworkLink {
     ...
     * 
     * <p>
     * <tt>nachos.conf</tt> specifies the reliability of the network. The reliability, between 0 and 1, is the
     * probability that any particular packet will not get dropped by the network.
     * 
     ...
     * 
     * <p>
     * The receive interrupt handler is called every time a packet arrives and can be read using <tt>receive()</tt>.
     * 
     * <p>
     * The send interrupt handler is called every time a packet sent with <tt>send()</tt> is finished being sent. This
     * means that another packet can be sent.
     * 
