 ...
 * 
 * <p>
 * The <tt>Processor</tt> API is re-entrant, so a single simulated processor can be shared by multiple user threads.
 * 
 * <p>
 * An instance of a <tt>Processor</tt> also includes pages of physical memory accessible to user programs, the size of
 * which is fixed by the constructor.
 */
public final class Processor {
     ...
     * 
     * <p>
     * When the exception handler is called, interrupts will be enabled, and the CPU cause register will specify the
     * cause of the exception (see the <tt>exception<i>*</i></tt> constants).
     * 
     ...
     * 
     * <p>
     * If <tt>false</tt>, this processor directly supports single-level paging; use <tt>setPageTable()</tt>.
     * 
     * <p>
     * If <tt>true</tt>, this processor has a software-managed TLB; use <tt>getTLBSize()</tt>, <tt>readTLBEntry()</tt>,
     * and <tt>writeTLBEntry()</tt>.
     * 
     * <p>
     ...
     * 
     * @param page the page number. Must be between <tt>0</tt> and <tt>(2<sup>32</sup> / pageSize) - 1</tt>.
     * @param offset the offset within the page. Must be between <tt>0</tt> and <tt>pageSize - 1</tt>.
     ...
     *            target are to be overwritten. If a bit in <tt>nextLoadMask</tt> is 0, then the
     *            corresponding bit of register <tt>nextLoadTarget</tt> will not be written.
     */
     ...
     * 
     * <p>
     * Transfer the contents of the nextPC register into the PC register, and then add 4 to the value in the nextPC
     * register. Same as <tt>advancePC(readRegister(regNextPC)+4)</tt>.
     * 
     * <p>
    ...
    public static final String exceptionNames[] = {
            "syscall      ", "page fault   ", "TLB miss     ", "read-only    ", "bus error    ", "address error",
            "overflow     ", "illegal inst " };
    ...
    private static class Mips {
        ...
        static final char RS = 's', RT = 't', RD = 'd', IMM = 'i', SHIFTAMOUNT = 'h', ADDR = 'a', // imm(rs)
                TARGET = 'j', RETURNADDRESS = 'r'; // rd, or none if rd=31; can't be last
        ...
        static final Mips[] optable = {
                ...
                new Mips(LUI, "lui ti", IFMT, DST | SRC2IMM | UNSIGNED), new Mips(), new Mips(), new Mips(),
                new Mips(), new Mips(BEQ, "beql stj", IFMT, BRANCH), new Mips(BNE, "bnel stj", IFMT, BRANCH),
        ...
        static final Mips[] specialtable = {
                new Mips(SLL, "sll dth", RFMT, DST | SRC1SH), new Mips(), new Mips(SRL, "srl dth", RFMT, DST | SRC1SH),
                new Mips(SRA, "sra dth", RFMT, DST | SRC1SH), new Mips(SLL, "sllv dts", RFMT, DST), new Mips(),
                new Mips(SRL, "srlv dts", RFMT, DST), new Mips(SRA, "srav dts", RFMT, DST),
                new Mips(JUMP, "jr s", RFMT, BRANCH), new Mips(JUMP, "jalr rs", RFMT, BRANCH | LINK | DST), new Mips(),
                new Mips(), new Mips(SYSCALL, "syscall "), new Mips(UNIMPL, "break "), new Mips(),
                new Mips(UNIMPL, "sync "), new Mips(MFHI, "mfhi d", RFMT, DST), new Mips(MTHI, "mthi s", RFMT, 0),
        ...
        static final Mips[] regimmtable = {
                new Mips(BLTZ, "bltz sj", IFMT, BRANCH), new Mips(BGEZ, "bgez sj", IFMT, BRANCH),
                new Mips(BLTZ, "bltzl sj", IFMT, BRANCH), new Mips(BGEZ, "bgezl sj", IFMT, BRANCH), new Mips(),
                new Mips(), new Mips(), new Mips(), new Mips(), new Mips(), new Mips(), new Mips(), new Mips(),
                new Mips(), new Mips(), new Mips(), new Mips(BLTZ, "bltzal sj", IFMT, BRANCH | LINK | DST | DSTRA),
